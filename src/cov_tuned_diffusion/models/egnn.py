...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(),
                 n_layers=4, recurrent=True, attention=True, norm_diff=True, out_node_nf=None,
                 tanh=False, coords_range=15, agg='sum', norm_constant=0, inv_sublayers=1, sin_embedding=False):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, E_GCL(self.hidden_nf, self.hidden_nf,
                                                self.hidden_nf, edges_in_d=in_edge_nf,
    ...
    def forward(self, h, x, edges, node_mask=None, edge_mask=None, edge_attr=None):
...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0,
                 act_fn=nn.SiLU(), attention=False, norm_diff=True, tanh=False,
                 coords_range=1, norm_constant=0):
    ...
    def edge_model(self, source, target, radial, edge_attr, edge_mask):  ## phi_e
    ...
    def node_model(self, x, edge_index, edge_attr, node_attr):  ## phi_h
        row, col = edge_index
    ...
    def coord_model(self, coord, edge_index, coord_diff, radial, edge_feat, node_mask, edge_mask):  ## phi_x
        row, col = edge_index
        if self.tanh:
    ...
    def coord2radial(self, edge_index, coord):
        ...
        norm = torch.sqrt(radial + 1e-8)  # only difference
